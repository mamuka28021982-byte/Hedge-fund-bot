...
# სრული და გაფართოებული სია ყველა ძირითადი კომპანიით და ფონდით
STOCKS = {
    "🏛 BIG TECH & AI GIANTS": [
        "NVDA",
        "MSFT",
    ...
    "⚛️ QUANTUM COMPUTING & TECH": ["IBM", "IONQ", "RGTI", "QBTS", "HON", "QNT"],
    "🚀 SPACE & DEFENSE": ["SPACE", "DXYZ", "RKLB", "LMT", "NOC", "KTOS"],
    "📈 MARKET ETFS & INDICES": ["QQQ", "SPY", "DIA"],
}
...
def get_stock_news(ticker):
  try:
    ...
    if news_list and len(news_list) > 0:
      latest = news_list[0]
      title = latest.get("title") or latest.get("content", {}).get(
          "title", "სიახლე არ მოიძებნა"
      )
      summary = (
          latest.get("summary")
          or latest.get("description")
          or latest.get("content", {}).get("summary", "")
      )
      ...
      try:
        ...
        logger.error(f"Title translation error: {trans_err}")
      ...
      if summary:
        try:
          ...
          logger.error(f"Summary translation error: {sum_err}")
...
async def start(update: Update, context: ContextTypes.DEFAULT_TYPE):
  try:
    await update.message.reply_text(
        ...
        "ბრძანებები:\n"
        "• `/analyze` - კომპანიებისა და აქციების ანალიტიკა\n"
        "• `/news` - უახლესი სიახლეები მოკლე შინაარსით (ქართულად)\n"
...
async def analyze(update: Update, context: ContextTypes.DEFAULT_TYPE):
  try:
    await update.message.reply_text(
        "⏳ მიმდინარეობს კომპანიების მონაცემების დამუშავება..."
    )

    header = (
        "🏛 **INSTITUTIONAL HEDGE FUND ANALYTICS**\n----------------------------------"
    )
    messages = [header]
    current_chunk = ""
...
async def news(update: Update, context: ContextTypes.DEFAULT_TYPE):
  try:
    await update.message.reply_text(
        "⏳ ვეძებ, ვთარგმნი და ვაანალიზებ კომპანიების უახლეს სიახლეებს..."
    )
    ...
    text = "📰 **კომპანიების უახლესი სიახლეები და შეჯამება**\n----------------------------------"

    for ticker in key_tickers:
      news_item = get_stock_news(ticker)
      if news_item:
        text += f"\n\n▪ **{ticker}**\n📌 **სათაური:** {news_item['title']}"
        if news_item["summary"]:
          text += f"\n💡 **მოკლე შინაარსი:** {news_item['summary']}"
        if news_item["link"]:
          text += f"\n🔗 [სრული სტატია]({news_item['link']})"
    ...
    await update.message.reply_text(
        f"⚠️ შეცდომა სიახლეების დამუშავების დროს: {str(e)}"
    )
